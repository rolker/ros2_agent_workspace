...
| 0013 — `progress.md` entry-type vocabulary | Adding a new workflow skill that writes to `progress.md`, or consuming prior entries from a skill | Use the canonical entry types (`## Issue Review`, `## Plan Authored`, `## Plan Review`, `## Local Review`, `## Local Review (Pre-Push)`, `## Integrated Review`, `## Implementation`); consume prior entries by entry-type filter, not by re-classifying |
