...
| `janitor-sweep` | Periodically | Chain the four staleness detectors into a workspace-scope and a project-scope report. **Local-first**: every run writes a local report file under `.agent/scratchpad/janitor/`, and committing `docs/health.md` via PR happens only with `--publish`. A project root that keeps a root `ROADMAP.md` also gets an optional second local report. Project scope is never published, pending the health-rollup shape decision |
