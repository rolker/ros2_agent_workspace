...
- **Roadmap and Governance are docs-website pages, not repo files**, even at the ros2 core level — tracked in `ros2_documentation`, with the live feature list on a GitHub Project board
- The pattern repeats across large ROS 2 projects: Nav2 → docs.nav2.org, MoveIt → moveit.ai, ros2_control → control.ros.org, Autoware → a separate `autoware-documentation` repo — none keep roadmap/vision/governance as in-repo files
...
- The one field/marine example found, `ros-maritime/community`, puts governance directly in `README.md` (no separate file) and tracks roadmap via a GitHub Project board plus a public Google Doc
