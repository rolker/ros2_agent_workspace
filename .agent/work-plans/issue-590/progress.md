...
- [x] (suggestion, Copilot) "No findings format" template's `**Local Adversarial**` line omits the `--local` token, unlike the adjacent Copilot line and unlike the standard/Light templates which use `run (<model>, --local) | skipped (<reason>, --local)`. Fix: add the flag token to both branches — `.claude/skills/review-code/SKILL.md:993`
