...
| Workspace vs. project separation | The two-root rule *is* this principle expressed as structure: the workspace assumes two roots and nothing else; everything below the project root is discovered by convention, and its absence is normal (ADR-0003). The kind → expected-location table is written as generic ROS 2 convention and the roots are named abstractly ("the repo the manifest points at"), so the workspace stays usable by any ROS 2 project; `unh_marine_autonomy` / `unh_echoboats_project11` are examples and first instances, not the source of the convention. |
