...
| (2) | Conventional-path discovery in the skills | Make `janitor-sweep` / `audit-project` find a repo's planning documents by probing a short list of **conventional paths** — the ones the workspace roots themselves use (`VISION.md`, `docs/roadmap.md`, `docs/decisions/`, the health document beside the roadmap) — instead of guessing per repo. **No new per-repo file and no schema**: the operator decided (2026-09-17) that expected locations following common conventions replace a declaration file. **Graceful absence is the headline requirement and gets an explicit test case**: a repo with none of those paths behaves exactly as it does today, never errors — ADR-0017's incremental-rollout stance. Ships the kind-marker convention and the `.agent/templates/` kind marker. If the probe is worth factoring into a shared script, it gets its own `AGENTS.md` Script Reference row; that is (2)'s call, not a requirement of (1). |
...
1. **Write `docs/decisions/0020-planning-document-kinds-and-the-two-root-rule.md`** (Status: Accepted) covering, in one document:
   ...
   - **Discovery by convention, not by declaration** — the ADR states the two-root rule and "a roadmap names the roadmaps beneath it", and says that below the project root a skill **probes a short list of conventional paths** (`VISION.md`, `docs/roadmap.md`, `docs/decisions/`, the health document beside the roadmap) and treats absence as "behave as today", never an error. **No per-repo declaration file and no schema is introduced** — the operator decided this (2026-09-17), against the alternative of a small declaration file on the `.agents/deployment.yaml` / ADR-0014 precedent, which the ADR records as the option considered and not taken (ADR-0017's own Negative for that precedent is "one more per-repo file"). The ADR states the convention and the graceful-absence requirement; **the skills that read it are sub-issue (2)**.
...
- **Discovery mechanism — DECIDED (operator, 2026-09-17): expected locations following common conventions; no new per-repo file, no schema.** Quoted: *"Maybe the skill can look at expected locations for files, and those expected locations should probably follow common conventions."* This closes the round-1 suggestion that was deferred to the operator at plan review and, per the round-2 review, was recorded only in `progress.md` and not here. ADR-0020 therefore states the convention and graceful absence rather than a declaration file; sub-issue (2) shrinks to the conventional-path reader plus its graceful-absence test, and the synthetic-fixture and sibling-manifest-comparison items proposed at the round-2 checkpoint are dropped with it.
