...
1. **Write `docs/decisions/0020-planning-document-kinds-and-the-two-root-rule.md`** (Status: Accepted), **structured from the blank ADR template in [`.agent/templates/adr_template.md`](../../../.agent/templates/adr_template.md) § "Blank ADR template"** (Status / Context / Decision / Consequences), extended with the **Alternatives considered** and **References** sections the recent ADRs carry (`docs/decisions/0019-what-contains-a-dispatched-agent.md` has both; verified 2026-09-17) — this ADR needs Alternatives for the two trigger mechanisms not chosen and the declaration-file option not taken. Covering, in one document:
...
| File | Change |
|------|--------|
| `docs/decisions/0020-planning-document-kinds-and-the-two-root-rule.md` | New ADR, from `.agent/templates/adr_template.md`'s blank template plus Alternatives considered / References (kinds, two-root rule, discovery by convention, publish=commit, bot identity, human-review gate, trigger decision, roadmap+health rule) |
