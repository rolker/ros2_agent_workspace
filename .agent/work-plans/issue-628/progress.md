...
- [ ] (low, Copilot R4) The ADR Applicability table says "(2)'s graceful-absence test **is the enforcement** of that". Round 1 established the opposite for the draft (`3e08c76`: #634 makes the two-root documents discoverable, not checkable) — the reader validates nothing and raises no finding, so the test is a regression guard on the project-agnostic guarantee, not enforcement of the rule. Same stale-copy class as the two above; restate as a regression guard — `.agent/work-plans/issue-628/plan.md:257`
