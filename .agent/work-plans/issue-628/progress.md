...
**PR**: #638 at `cb0fe52`
**Sources**: 5 (Copilot R1 @ `92526cf`, R2 @ `0829d73`, R3 @ `aa05f55`, R4 @ `cb0fe52`; prior Integrated Review @ `0829d73` + Local Review @ `aa05f55`; CI rollup)
**Cross-source confirmations**: 0 at head `cb0fe52` — Copilot is the only live source; the local timeline's own head-`aa05f55` finding was fixed by `cb0fe52`. Three items are re-raised by Copilot across R3 and R4, which is one source twice, not two sources.
**CI**: all-pass (9 checks: Lint (pre-commit), Validate Documentation, Script tests, Validate commit identity (Mechanism C), copilot reviewer — no failures, none pending)

Round 2. The round-1 `## Integrated Review` @ `0829d73` (10 findings, fixed in `b1b3882`..`883b7f3`) and the `## Local Review` @ `aa05f55` (fixed in `cb0fe52`) are the baseline; everything they resolved and Copilot re-raised from a stale head is recorded below as addressed, not re-opened. The live material is R3 @ `aa05f55` and R4 @ `cb0fe52`, and it falls into two groups: one **real functional gap** in `.claude/skills/inspiration-tracker/SKILL.md` — the consumer this PR repointed at root `ROADMAP.md` — and a set of **stale copies in `plan.md`** of text whose deliverable-side original was already corrected on this branch.

### Findings
- [x] (must-fix, Copilot R3+R4) The roadmap row this PR taught `inspiration-tracker` to write is never committed. Step 8 now appends a **Deferred** row to root `ROADMAP.md`, but step 9 stages only the digest (`git add .agent/knowledge/inspiration_<name>_digest.md`) and step 10 pushes that one commit and opens the PR. Verified against the file: no other step stages `ROADMAP.md`. Before this PR the write target (`docs/ROADMAP.md`) did not exist, so the whole path was inert; repointing it at a real file is what makes the loss live — an accepted finding is written, left unstaged in a skill worktree that is then removed, and silently vanishes. Stage `ROADMAP.md` alongside the digest when the user chose "Add to roadmap" (a conditional `git add`), and say so in step 10's PR body — `.claude/skills/inspiration-tracker/SKILL.md:264,277-283`
- [x] (must-fix, Copilot R3) The emitted Deferred row carries a literal placeholder in its third column. The row template writes `<the condition that would bring it back>` into the **Deferred because** column, but step 7 asks the user only Add / Skip / Defer — nothing collects or derives that reason, so an accepted finding produces a malformed published row in the canonical roadmap. This is the same failure class as the finding above (the flow writes into a real document now), and the workspace's own Deferred table gives real reasons in that column (`ROADMAP.md:112-114`). Prompt for the reason when "Add to roadmap" is chosen, or derive it from the digest entry — `.claude/skills/inspiration-tracker/SKILL.md:267`
- [x] (must-fix, Copilot R3+R4) The as-built Estimated Scope records "**six** small edits" and names six files, and adds "Rev 4's eight edits became six". The delivered diff edits **seven** existing files: the six named plus `.claude/skills/inspiration-tracker/SKILL.md`, which the round-1 fix added to migrate the roadmap consumer. The plan-first workflow requires the as-built record to match the branch, and this is the sentence a reviewer reads to check the diff's shape. Update the count, the "those six files" and "became six" wording, and note the consumer migration as a round-1 review addition — `.agent/work-plans/issue-628/plan.md:328`
- [x] (low, Copilot R3+R4) The plan's copy of the kind → expected-location table still labels the core kind `decisions` while its own row-by-row bullet below says "**decision — settled**" and the shipped draft uses the singular. Round 1 fixed the singular in the draft (`docs/design/planning_document_vocabulary.md:114`) and did not carry it into the plan copy. #634 derives its kind marker from this table, so two spellings on one branch leave the marker ambiguous. Singular kind, plural directory path — `.agent/work-plans/issue-628/plan.md:90`
- [x] (low, Copilot R3+R4) The plan still ends the graceful-absence paragraph with "the skills simply do not find it and behave exactly as they do today", the exact sentence `883b7f3` corrected in the draft after round 1 flagged that #634 adds discovery-only fallbacks (org board, `homepage`, README links) that can surface a document outside the expected paths. Carry the draft's clause across: the canonical in-repo probe is what the table promises, the fallbacks are optional discovery, and absence is never a finding either way — `.agent/work-plans/issue-628/plan.md:95`
- [x] (low, Copilot R3+R4) Three places in the plan still describe the vision and roadmap rows as unresolved, against the operator decision of 2026-09-14/17 that settled them (README § Vision; root `ROADMAP.md`) and against the plan's own status column at lines 88-89. The evidence bullet opens "**Evidence, and which rows are still open — the table is provisional**" and says "the reason two rows stay open rather than settled" (line 99); the Consequence line makes "resolving the two open rows" part of #637's promotion gate (line 107); the Principles Self-Check says the rows are "**marked open**" (line 247). Copilot is not re-opening the decision here — it is reporting that the plan contradicts it, which would hand #637 a requirement that no longer exists. Rewrite all three as rationale *for* the settled rows — `.agent/work-plans/issue-628/plan.md:99,107,247`
- [x] (low, Copilot R4) The ADR Applicability table says "(2)'s graceful-absence test **is the enforcement** of that". Round 1 established the opposite for the draft (`3e08c76`: #634 makes the two-root documents discoverable, not checkable) — the reader validates nothing and raises no finding, so the test is a regression guard on the project-agnostic guarantee, not enforcement of the rule. Same stale-copy class as the two above; restate as a regression guard — `.agent/work-plans/issue-628/plan.md:257`
- [x] (low, Copilot R3) The #637 promotion gate lists three conditions, all about evidence, and says nothing about the link migration promotion forces. Verified: eight tracked files outside the work-plan directory link to `docs/design/planning_document_vocabulary.md` (`ROADMAP.md`, `.agent/templates/roadmap.md`, `.agent/knowledge/principles_review_guide.md`, the four instruction files, `.claude/skills/inspiration-tracker/SKILL.md`). Moving the draft to a numbered ADR breaks all eight. One clause on the gate — promotion carries the pointer updates — costs nothing now and is exactly the "check the consequences" step AGENTS.md § Post-Task Verification asks for — `docs/design/planning_document_vocabulary.md:414-421`
- [x] (low, Copilot R3 — corrected here, not edited) The round-1 Verification note says `ROADMAP.md` "stays at its pre-existing size, **inside** the 150-line bound"; it measures exactly **150**, i.e. at the bound, not inside — the third recurrence of the inside/at distinction this PR has already chased twice. `progress.md` is an append-only timeline (ADR-0013) and the round-1 pass deliberately recorded that no past entry was edited, so the correction belongs in this entry rather than in a rewrite of that one. Recorded: `ROADMAP.md` is 150 lines at `cb0fe52`, which **meets** the 150-line first-cut bound; the plan's as-built record already states the bound and defers the count to publish, which is the durable form AGENTS.md § Documentation Accuracy wants — `.agent/work-plans/issue-628/progress.md:824`

### Addressed by earlier commits on this branch (re-raised from stale heads)
- (Copilot R1 @ `92526cf`, `plan.md:365`) As-built still claims six `#TBD` placeholders and unfiled sub-issues — fixed in the round-1 pass; the as-built record now reads as a past-tense publish-time staging note and every `#TBD` was replaced with a real link (#634-#637, `unh_marine_autonomy#394`, `unh_echoboats_project11#492`).
- (Copilot R2 @ `0829d73`, `ROADMAP.md:57` and `docs/design/planning_document_vocabulary.md:113`) `inspiration-tracker` still writes to `docs/ROADMAP.md`'s "To Consider" section — fixed in the round-1 pass; all three lines now name the root `ROADMAP.md` **Deferred** section. The remaining gap in that same flow (staging) is finding 1 above and is new material, not a repeat.

### False positives
- (Copilot R4 @ `cb0fe52`, review body) "The work plan contains stale inconsistencies, and the inspiration tracker does not stage updated roadmap entries" is accurate, but R4's headline framing **🔵 Needs a closer look** lists no new comment ("Comments generated: 0 new") and its seven suppressed items are the R3 set plus one previously-missed line — the PR did not regress between `aa05f55` and `cb0fe52`. No finding is dismissed on this basis; recorded only so a later reader does not count R4 as an independent confirmation of R3.

### Assessment
Eight open findings, three must-fix. All three must-fix items are small and local: two are one-step edits inside `.claude/skills/inspiration-tracker/SKILL.md` (stage the roadmap edit; collect the deferral reason) and one is an as-built count in `plan.md`. The other five are stale copies in `plan.md` of text already corrected on the deliverable side during round 1, plus a one-clause addition to the draft's promotion gate — they can all ride one commit. Nothing touches the operator decisions recorded on #628, and no finding argues against the design. CI is green on every check. The PR is **not** merge-ready as it stands, because the roadmap-staging gap would silently drop an accepted finding the first time `/inspiration-tracker` runs after this merges; with that and the two other must-fix items fixed, it is.

### Fix plan
1. `.claude/skills/inspiration-tracker/SKILL.md` — step 7/8: prompt for the deferral reason when "Add to roadmap" is chosen; step 9: stage `ROADMAP.md` with the digest when that choice was made; step 10: mention the roadmap row in the PR body.
