...
### Findings
- [ ] (must-fix) Check 1 is the only check not anchored at `$ROOT`: step 1 mandates `$ROOT` for "**every** script and file the sweep touches" and `:304-306` lists check 1's inputs as `$ROOT/...` paths, but it delegates to `/audit-workspace`, which addresses everything by bare relative path (`audit-workspace/SKILL.md:31,55,64,73,80`) and audits **cwd** — so a sweep run from a worktree (the skill's own stated normal case) grades that branch's governance docs for check 1 and the main root for checks 2-4, and reports the mixture as one workspace state — `.claude/skills/janitor-sweep/SKILL.md:295,304-310`
- [ ] (must-fix) The "does this workspace have its own manifest?" early return is narrower than `list_overlay_repos.py`'s own search path — it returns only on `configs/manifest/repos`, while `get_overlay_repos` also reads `configs/*.repos` (`lib/workspace.py:89-92`) — so on that supported layout the helper attempts an unneeded network clone and `resolve_repo_checkout.sh:225` turns its rc 5 into `exit 5` BEFORE the manifest on disk is ever read; reproduced in a sandbox (same tree resolves fine with the bootstrap pointer removed, hard-fails at exit 5 with it present) — a false RED over a workable state — `.agent/scripts/manifest_fallback.sh:64`
- [ ] (must-fix) A manifest that could not be REFRESHED is used silently as far as the report is concerned: the helper warns on stderr and returns 0 with the cached clone, both callers capture stdout only, and the status contract has no state for it — so a sweep can report `4 of 4 completed` over a repo list enumerated from a manifest it could not refresh, the report-level false green the contract exists to prevent; the origin re-clone guard also keys on url only, not branch (Governance + Lens A) — `.claude/skills/janitor-sweep/SKILL.md:105-118`, `.claude/skills/issue-triage/SKILL.md:53-62`, `.agent/scripts/manifest_fallback.sh:150-157`
- [ ] (suggestion) The `$ROOT` defect deferred in round 3 has a second site with a false rationale attached: the comment says "in a layer worktree neither `.agent/scripts/` nor `layers/` sits beside you", but step 1's own caveat (`:48-50`) says `$ROOT` resolves to the PROJECT repo's root there, so the `$ROOT` paths are exactly as absent as the relative ones they replaced — the deferral should cover both sites or neither (fix-every-site rule) — `.claude/skills/audit-project/SKILL.md:180-183`
- [ ] (suggestion) The bare enumeration command still precedes the fallback snippet, so on a no-`layers/` host it prints `[]` at exit 0 and the guard maps that to the wrong remedy; `janitor-sweep` was restructured so enumeration can only happen on the fallback's success path, `issue-triage` was not — `.claude/skills/issue-triage/SKILL.md:41` vs `:74-78`
- [ ] (suggestion) A legal nested vcstool manifest key (`tools/foo`) is reported as a USAGE error blaming the caller, although `janitor-sweep` feeds names straight out of the manifest; name it as a manifest-shape limitation instead. Latent — no manifest here uses one today — `.agent/scripts/resolve_repo_checkout.sh:109`
- [ ] (suggestion) `--repos` has no rule for a name that is in no manifest, so `/janitor-sweep --repos typo_name` lands on rule 4 and reports `SKIPPED(no eligible repos)` for a repo the operator explicitly asked for; `issue-triage`, edited in the same PR, states the analogous rule — `.claude/skills/janitor-sweep/SKILL.md:14-20`
- [ ] (suggestion) `RESOLVE_LOCK_TIMEOUT` reaches `flock -w` unvalidated: a non-numeric value makes flock exit 64 immediately and the script prints "could not lock ... within abcs — another run may be wedged", sending the operator after a phantom process — one fix with the deferred `exec 9>` misdirection — `.agent/scripts/resolve_repo_checkout.sh:381-386`
- [ ] (suggestion) The manifest helper's three url messages are unredacted while the resolver has `redact_url` for exactly this funnel-into-the-report reason, and the accepted `WORKSPACE_MANIFEST_GIT_BASE` form admits `https://user:pw@host` — `.agent/scripts/manifest_fallback.sh:145,161,168`
- [ ] (suggestion) Two snippets use `continue` / `return` outside any loop or function and are bash errors if run verbatim; both are illustrative, but every other snippet in the file is copy-pasteable — `.claude/skills/janitor-sweep/SKILL.md:188,282`
- [ ] (suggestion) The `manifest_fallback` row says exit 5 covers "the clone **or refresh** failed"; the code returns 0 on a refresh failure with a stale-cache warning, and the script's own header gets this right — AGENTS.md is the single outlier, and it is wrong in the false-green direction (the doc side of must-fix 3) — `AGENTS.md:585`
- [ ] (suggestion) The inline exit-5 gloss omits the fourth cause, a layer checkout that exists and cannot be read (`resolve_repo_checkout.sh:181-184,192-195`) — the one case where "fall back to a clone" would be wrong; AGENTS.md:584 has it — `.claude/skills/audit-project/SKILL.md:88-89`
- [ ] (suggestion) The usage block and function list still show the pre-change signatures (`get_overlay_repos(include_underlay=False)`, no `--config-dir`); weak bar, since that README is already non-exhaustive — `.agent/scripts/README.md:562-570,611-619`
- [ ] (suggestion) The sweep's read-only workhorses (`resolve_repo_checkout.sh`, `list_overlay_repos.py`) are not allowlisted while peers are, so an unattended run prompts per repo — correctly out of scope while the trigger is deferred, flagged as an input to that decision — `.claude/settings.json`
- [ ] (suggestion) The resolver's "the lock is released at exit, so the caller reads `$TARGET` unlocked — concurrent audits of the SAME repo are not safe" caveat is carried to neither calling skill; the visible result of losing that race is a PHANTOM finding (`MISSING: no root AGENTS.md`) indistinguishable from a real one, and several agents run at once on this host — `.claude/skills/janitor-sweep/SKILL.md:145-154`, `.claude/skills/audit-project/SKILL.md:56-95`
- [ ] (suggestion) Retention caps the 20 small reports and never prunes the clone caches, which are the output that actually consumes disk (one shallow working tree per repo ever resolved, including repos later dropped from the manifest) — on a host whose dev disk has already hit 100% — `.claude/skills/janitor-sweep/SKILL.md:145-154`
- [ ] (suggestion) Re-grade the deferred trust-model item before merge: ADR-0019 §3 says a phase that fetches its own text "must carry the fence in its own SKILL.md", this sweep fetches its own text by design, and clones land INSIDE the agent's own tree (`$MAIN_ROOT/.agent/scratchpad/janitor-repos/<repo>`) where instruction files auto-load — a stronger exposure than recorded — `.claude/skills/janitor-sweep/SKILL.md`, `.agent/scripts/resolve_repo_checkout.sh:358`
- [ ] (suggestion) The plan under-states the AGENTS.md Ask-First edits: the row names only the resolver, while the diff makes FOUR changes (resolver row, new `manifest_fallback.sh` row, rewritten `field_mode.sh` row, and a 17-line § Field Mode prose block added in round 3 carrying a normative empty-url sentence). Open Questions tells the operator to re-confirm "two details" — the § Field Mode prose is a third, later addition and appears nowhere in the plan, so the re-confirmation checklist is incomplete — `.agent/work-plans/issue-569/plan.md:299,305,355-366`
- [ ] (suggestion) No round-3 plan-revision narrative, unlike rounds 1 and 2: round 3's changes exist only as "Round 3 —" clauses inside three Files-to-Change rows, so a future agent reading top-to-bottom sees two review rounds where the timeline records three — `.agent/work-plans/issue-569/plan.md:37,67`
- [ ] (suggestion) `## Documentation & Instruction Impact` omits `.claude/skills/issue-triage/SKILL.md`, which the diff rewrites substantially (+60/−2, a chained skill's failure vocabulary) — it is in Files-to-Change, but Doc-Impact is the section a reviewer reads for "which instruction/procedure files does this PR touch" — `.agent/work-plans/issue-569/plan.md:344-347`
- [ ] (suggestion) Still open from round 3 and shipping with this PR: `.agent/scratchpad/README.md`'s `find .agent/scratchpad -type f -mtime +1 -delete` advice would gut the janitor caches and contradicts the keep-last-20 retention — the caches self-heal (both origin checks re-clone on a broken tree), the reports do not — `.agent/scratchpad/README.md:82-83`
