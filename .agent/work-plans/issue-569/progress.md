...
- [ ] (cross-confirmed: Copilot R2 suppressed + Local Review R3 @ `9dc555f`, suggestion) Same defect's other half: the layer-checkout glob accepts the first non-empty `layers/main/*/src/<name>` directory without reading the manifests at all, so a renamed or re-pointed repo, or a checkout sitting at an old pin, is audited as the current repo — unlike the cached-clone path, which verifies its origin. Fix alongside the item above (verify origin/pin before accepting a layer checkout, or state in the contract that a layer checkout is accepted as-is and report the mode so the reader knows the pin was not checked) — `.agent/scripts/resolve_repo_checkout.sh:175-197`
