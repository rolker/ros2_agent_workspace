...
## Integrated Review
**Status**: complete
**When**: 2026-09-14 08:34 -04:00
**By**: Claude Code Agent (Claude Opus)

**PR**: #625 at `9dc555f`
**Sources**: 3 (Copilot R1 @ `71bf3e1`, Copilot R2 @ `9dc555f`, the four local review rounds in this file — Local Review R1-R3 + Local Review (Pre-Push) R4 — plus the CI rollup)
**Cross-source confirmations**: 5
**CI**: all-pass (9 checks; `Script tests`, `Lint (pre-commit)`, `Validate Documentation`, `Validate commit identity`, `copilot-pull-request-reviewer` all success — the one `skipped` is the duplicate identity job on the earlier workflow run)

Nine Copilot inline comments across two passes. Three (`manifest_fallback.sh`
local-manifest guard, refresh-failure rc, check-1 cwd anchoring) are outdated +
resolved — verified fixed in the current tree by commits `8e1f779`, `1cb2113`,
`0f9345c` respectively. Six threads remain open; five of them re-raise items
this file already recorded as deferred, which makes them cross-source
confirmations rather than new information.

### Findings
- [x] (cross-confirmed: Copilot R1 + Local Review R4 @ `71bf3e1`, must-fix) Credentials can reach stderr and the sweep report: all three `manifest_fallback.sh` url diagnostics interpolate `$git_url` verbatim (clone failure, layout disagreement, cached-origin mismatch) while the accepted `WORKSPACE_MANIFEST_GIT_BASE` form admits `https://user:pw@host` and `ssh://user:pw@host`; `resolve_repo_checkout.sh:126` already has `redact_url` for exactly this funnel. Fix: give the manifest helper the same `redact_url` (shared or duplicated) and route every url-bearing message through it — `.agent/scripts/manifest_fallback.sh:171,190,201`
- [x] (cross-confirmed: Copilot R2 + Local Review R3 @ `9dc555f`, must-fix) Same funnel, second site and a different carrier: the resolver redacts `$REPO_URL` but interpolates git's own captured output `$out` verbatim, and git echoes the full remote url — userinfo included — in many clone/fetch errors. R3 flagged the adjacent leak (absolute paths in the same strings, deferred "next round"); the fix-every-site rule makes these one pass over the reason strings that `audit-project` funnels into `FAILED: ...` and the sweep records in a report step 4 requires to be free of host identity. Fix: sanitize captured command output (a `redact_text` that strips `scheme://userinfo@` anywhere in the string) and strip `$MAIN_ROOT`/`$HOME` prefixes from paths — `.agent/scripts/resolve_repo_checkout.sh:419,426` (and the sibling messages at `:182,186,193,385,448,455`)
- [x] (cross-confirmed: Copilot R2 + Local Review R3 @ `9dc555f`, must-fix) The manifest-pinned `version:` is not honoured deterministically: `AMBIGUOUS` keys on the url set only, so the same repo declared in two manifests at the same url with different `version:` pins takes `matches[0]` in sorted-manifest order and the clone silently audits the wrong branch/commit — the exact asymmetry exit 7 exists to refuse, and silent, not loud. Fix: treat conflicting `(url, version)` pairs as `AMBIGUOUS` (message naming both pins and their source files) and add a regression case to `.agent/scripts/tests/test_resolve_repo_checkout.sh` — `.agent/scripts/resolve_repo_checkout.sh:270-276`
- [x] (cross-confirmed: Copilot R2 suppressed + Local Review R3 @ `9dc555f`, suggestion) Same defect's other half: the layer-checkout glob accepts the first non-empty `layers/main/*/src/<name>` directory without reading the manifests at all, so a renamed or re-pointed repo, or a checkout sitting at an old pin, is audited as the current repo — unlike the cached-clone path, which verifies its origin. Fix alongside the item above (verify origin/pin before accepting a layer checkout, or state in the contract that a layer checkout is accepted as-is and report the mode so the reader knows the pin was not checked) — `.agent/scripts/resolve_repo_checkout.sh:175-197`
- [x] (cross-confirmed: Copilot R1 + Local Review R3/R4 @ `71bf3e1`, should-fix) `audit-project`'s `ROOT` resolves to the **project repo's** root when the skill runs from the documented layer worktree, so `$ROOT/.agent/scripts/resolve_repo_checkout.sh` does not exist (named-repo audits fail) and `case "$REPO_PATH" in "$ROOT"/layers/*/src/*` misreads a genuine layer checkout as mode `clone` (silently wrong, conservative). The current tree documents the caveat in prose at `:49-52` and `:172-177` but nothing acts on it. Fix: discover the workspace root properly (walk up for `configs/project_bootstrap.url` + `.agent/scripts/`, or accept an explicit `--workspace-root`/`$WORKSPACE_ROOT`) instead of asking the operator to remember — `.claude/skills/audit-project/SKILL.md:47-73,172-183`
- [x] (cross-confirmed: Copilot R2 + Local Review R3 @ `9dc555f`, should-fix) The fallback derives the manifest repo, branch and config path from the raw pointer path, while `setup_layers.sh:173-184` treats `bootstrap.yaml`'s `git_url:`/`branch:`/`config_path:` as authoritative (defaulting `config_path` to `config`). A bootstrap whose `git_url` names a non-GitHub host, or whose `config_path` differs from where `bootstrap.yaml` itself sits, then clones the wrong place — and the operator is told "the bootstrap pointer and the manifest repo disagree" (exit 5), which blames a disagreement that is really this derivation's limit. Loud, not a false green, but wrong about the cause. Fix (cheap, and it removes the round-3 chicken-and-egg objection): after the clone, read the now-local `$clone_dir/<derived path>/bootstrap.yaml` with the same `grep | awk` `setup_layers.sh` uses — no network, no YAML parser — honour its `config_path`, and fail with an accurate reason when `git_url`/`branch` disagree with what was derived — `.agent/scripts/manifest_fallback.sh:104-124,200-203`
- [x] (Copilot R2, nit) The report filename `$(date '+%Y%m%dT%H%M%S')-sweep.md` is second-precision, so two runs finishing in the same second overwrite each other although the section states the runs are the record and must accumulate. Low likelihood (a sweep is minutes long, so this needs concurrent runs — which the clone-cache locks otherwise make safe), and the fix is one token: add `%N` or `$$` to the name, or refuse to overwrite an existing path — `.claude/skills/janitor-sweep/SKILL.md:375`

### False positives
- (none) Every open Copilot thread verified against the current tree reproduces as described. The three resolved+outdated threads were re-read at their current lines rather than trusted: `manifest_fallback.sh:79-86` now returns early for `configs/*.repos` (excluding `underlay.repos`), `:157-176` returns 6 on a failed refresh instead of warning over a success, and `janitor-sweep/SKILL.md:312-318` anchors check 1 with `cd "$ROOT"` so all four checks grade one tree.
...
### Findings
- [x] (must-fix) The "or set WORKSPACE_ROOT" remedy is inert on the path it is printed for — the inline walk-up is what locates the script, and $WORKSPACE_ROOT is only read inside it — `.claude/skills/janitor-sweep/SKILL.md:97`, `.claude/skills/issue-triage/SKILL.md:54`, `.claude/skills/audit-project/SKILL.md:66,107`
- [x] (must-fix) With WORKSPACE_MANIFEST_GIT_BASE unset, the bootstrap.yaml `git_url` is compared as a raw string, so scp-form / no-`.git` / trailing-slash urls hard-fail at exit 5 as "not the one its own bootstrap names"; `_manifest_fallback_repo_path` exists for exactly this and is applied only in the other branch, which is the only one tested — `.agent/scripts/manifest_fallback.sh:273-283`
- [x] (must-fix) The suite is not hermetic against an ambient `$WORKSPACE_ROOT` despite its header claiming so — 4 of 7 cases fail with it set, in the environment the three skills now tell operators to create — `.agent/scripts/tests/test_workspace_root.sh:14-21`
- [x] (must-fix) A missing `redact.sh` returns 2, colliding with the documented "2 = executed rather than sourced"; the resolver uses 5 for the identical condition, and with the `source` rc unchecked and no `*)` arm the condition surfaces as exit 3 "run make setup-all" — the wrong remedy — `.agent/scripts/manifest_fallback.sh:85`, `.agent/scripts/resolve_repo_checkout.sh:260-275`
- [x] (must-fix) Both skills source `manifest_fallback.sh` without setting `REDACT_PATH_PREFIXES`, so its diagnostics keep absolute host paths and are then transcribed into a report the same file requires free of them — `.claude/skills/janitor-sweep/SKILL.md:120`, `.claude/skills/issue-triage/SKILL.md:73`
- [x] (suggestion) Two shapes of one snippet ship together: the header's usage block keeps walking on failure (`&& break`), the three skills and the test break unconditionally — `.agent/scripts/workspace_root.sh:33-38`
- [x] (suggestion) The `[ -x ]` gate makes the walk depend on exec bits (noexec volume, archive unpack, CIFS); `[ -f ]` + `bash "$d/…"` removes it — three SKILL.md walk-ups
...
- [x] (suggestion) "Authoritative about git_url:/branch:/config_path:" overstates — only `config_path` is honoured; the other two are hard-failing cross-checks — `.agent/scripts/manifest_fallback.sh:13-17`, `AGENTS.md:585`
...
- [x] (suggestion) The main-checkout hop silently redirects even an explicitly-set `$WORKSPACE_ROOT` — a consequential default worth one clause — `.agent/scripts/workspace_root.sh:80-85`
...
- [x] (suggestion) § Documentation & Instruction Impact names only `audit-project` among the skills; round 5 changed three — `.agent/work-plans/issue-569/plan.md:387-394`
...
## Local Review (Pre-Push)
**Status**: complete
**When**: 2026-09-14 09:40 -04:00
**By**: Claude Code Agent (Claude Opus)
**Verdict**: changes-requested

**Branch**: feature/issue-569 at `5ac4c79`
**Mode**: pre-push (targeted re-read of the round-5 fix pass, `47e832a..HEAD`; PR #625 open at `9dc555f`, 19 commits unpushed)
**Depth**: Deep (credential handling, AGENTS.md + three SKILL.md governance triggers — carried from round 5)
**Must-fix**: 2 | **Suggestions**: 11 new (14 deferred from round 5 still open, not re-litigated)
**Round**: 6 | **Ship**: recommended — must-fix fell 5 → 2, both are precise one-line corrections with no design question in them: normalise a relative $WORKSPACE_ROOT at five copies of one snippet, and correct one sentence in the plan. Another full round is not what either needs; the hang fix does need a regression case and a re-read of that one change.

**All five round-5 must-fixes are genuinely closed** — verified independently at every site, not taken from the implementation entry. The walk-up snippet is now byte-identical across all four copies (diffed mechanically). The `git_url` cross-check goes through `_manifest_fallback_url_key` on BOTH branches, and the branch no end-to-end case can reach without the network is unit-tested directly (8 cases). `test_workspace_root.sh` passes 10/10 with an ambient $WORKSPACE_ROOT exported. `redact.sh` unusable answers 5 at all three callers, each checking the `source` rc and carrying a `*)` arm; `manifest_config_dir` returns only 0/3/5/6 and every caller covers all four. `REDACT_PATH_PREFIXES` is set before every source site, most-specific-first, and no path through the changed code prints a url or host path outside the redacting funnel.
...
- [ ] (suggestion) Added this pass, in the condition class this pass fixed elsewhere: the resolver's two `redact.sh` refusals print the absolute `$SCRIPT_DIR/redact.sh` through a bare `echo`, while `manifest_fallback.sh:107` — added this pass for the identical condition — deliberately names no path. The resolver's stderr is transcribed into the sweep report — `.agent/scripts/resolve_repo_checkout.sh:131,141`
- [ ] (suggestion) Within one copy-paste block the failure arms disagree: the new `if ! source` arm exits, the four `case "$rc"` arms only echo. True of the snippet only because `fi` is the last statement in the fence — `.claude/skills/janitor-sweep/SKILL.md:156-159`, `.claude/skills/issue-triage/SKILL.md:104-107`
- [ ] (suggestion) `mkdir -p "$REPORT_DIR" || echo "FAILED(...)"` does not exit, unlike every other FAILED arm the file requires to be terminal, and it prints an absolute host path — `.claude/skills/janitor-sweep/SKILL.md:108`
- [ ] (suggestion) The file-ABSENT sibling of must-fix 4 still only `say`s and falls through to the empty-enumeration exit 3; the source-failure sibling now exits 5. Weaker than must-fix 4 (falling through is defensible when the workspace has its own manifest), but the two arms of one `if` answer differently — `.agent/scripts/resolve_repo_checkout.sh:306-308`
- [ ] (suggestion) The `redact.sh` row still says the callers refuse "when it is **missing**"; this pass widened both to "missing or will not load" and gave the condition exit 5. It is the only row describing this contract without an exit code — `AGENTS.md:587`
- [ ] (suggestion) The `workspace_root.sh` row was not touched and now understates two properties the header newly calls out as deliberate: the main-checkout hop is gated on the destination being a workspace root, and it redirects even an explicitly-set $WORKSPACE_ROOT — `AGENTS.md:584`
- [ ] (suggestion) § Documentation & Instruction Impact names only the AGENTS.md script table; the Field Mode addition belongs in it too (twin of must-fix 2) — `.agent/work-plans/issue-569/plan.md:428-436`
- [ ] (suggestion) `${extra:+--config-dir "$extra"}` expands unquoted, so a $ROOT containing a space word-splits the path; the resolver's own equivalent is already array-safe — `.claude/skills/issue-triage/SKILL.md:98`, `.claude/skills/janitor-sweep/SKILL.md:147`

### Carried forward, not re-litigated
The 14 suggestions the host deferred in round 5 remain open and unchecked in that entry. One of them was **reproduced** here and is worth taking with the must-fix pass, since it is the exact sibling of round-5 must-fix 3: `test_resolve_repo_checkout.sh` is not hermetic against an ambient `$BOOTSTRAP_URL` — **11 of 56 cases fail** with it exported (`WORKSPACE_ROOT` and `WORKSPACE_MANIFEST_GIT_BASE` are both clean, 56/56). It stays a suggestion rather than rising: unlike $WORKSPACE_ROOT, nothing in these skills tells an operator to set $BOOTSTRAP_URL.
...
## Integrated Review
**Status**: complete
**When**: 2026-09-14 10:34 -04:00
**By**: Claude Code Agent (Claude Opus)

**PR**: #625 at `d7b8baf`
**Sources**: 3 (Copilot R3 @ `d7b8baf`, Local Review (Pre-Push) @ `5ac4c79`/`2e2c16e` (rounds 5-6), Local Review (Pre-Push) @ `4f06a80` (round 4), CI rollup)
**Cross-source confirmations**: 2 (one same-head, one recurrence across heads)
**CI**: all-pass (9 checks green on `d7b8baf`; `copilot-pull-request-reviewer`, Lint, Script tests, Validate Documentation, Mechanism C)
**Round**: 3 of triage (prior `## Integrated Review` @ `9dc555f`); only the three threads opened by Copilot R3 are unresolved — every older thread is resolved.

### Findings
- [x] (must-fix, cross-confirmed — recurrence: Copilot R3 @ `d7b8baf` + Local Review round 4 @ `4f06a80`, where it was deferred by operator direction and is tracked in NO filed issue) The Retention block tells the operator the two cache directories are "disposable shallow clones, safe to `rm -rf` at any time", and instructs the report footer to repeat that. It is not true of the case the same block reasons about seven lines later: it declines age-based pruning precisely because "pruning by age would delete a clone a concurrent audit is reading", then calls the whole-directory `rm -rf` — which deletes strictly more, that clone included — the "cheap, safe reclaim". `resolve_repo_checkout.sh:94-101` states the underlying contract plainly: the per-repo `flock` covers clone/refresh only and is released at script exit, so the caller reads `$TARGET` unlocked and "concurrent audits of the SAME repo on one host are not safe". Consequence is the silent-wrong-result class, not a nit: an operator short of disk follows the footer mid-sweep, a concurrent audit reads a vanished or half-recreated tree, and a report-only skill emits a phantom `MISSING: no root AGENTS.md` indistinguishable from a real finding — on a host that routinely runs several agent sessions at once. Fix is documentation, not a new lock: state the real condition (safe when no sweep or audit is running on this host), carry the resolver's caveat into the skill, and make the footer say it. Copilot's alternative remedy — a shared lifecycle lock — is the caller-contract change `resolve_repo_checkout.sh` already scopes out, and is not warranted in a report-only skill nothing yet triggers — `.claude/skills/janitor-sweep/SKILL.md:206-221`
- [x] (cosmetic/defensive, cross-confirmed @ `d7b8baf`: Copilot R3 x2 + Local Review round 5/6 — **but both state a mechanism that is false**) Convert the two `${VAR:+--config-dir "$VAR"}` sites to explicit quoted array elements. Recommended only because the change is two lines with no behavioural risk and it retires a thread three reviews have now spent effort on — **not** because the code is wrong: see the false-positive entry below. If taken, correct the wording carried in #626 and in the round-5/6 entries at the same time, so the durable record stops asserting something untrue about bash — `.claude/skills/issue-triage/SKILL.md:102`, `.claude/skills/janitor-sweep/SKILL.md:152`

### False positives
- (Copilot R3 @ `.claude/skills/issue-triage/SKILL.md:102` and `.claude/skills/janitor-sweep/SKILL.md:152`; the same claim appears in the round-5/6 Local Reviews and in follow-up #626) "The `--config-dir` argument is expanded unquoted … a path containing spaces is split into multiple argv entries / array elements." The word-splitting cannot occur. In `${VAR:+--config-dir "$VAR"}` the alternate word is itself parsed for quoting before the result of the whole expansion is split: the inner `"$VAR"` is a quoted expansion and is exempt from both word splitting and pathname expansion, while the literal `--config-dir ` contributes exactly one word. Verified empirically on this host (bash 5.2.21 and dash) for an array assignment, a `set --`, and the empty case: `EXTRA_CONFIG="/path with space/manifest"` yields exactly two elements — `[--config-dir]`, `[/path with space/manifest]` — unchanged under a hostile `IFS=$'\n\t /'` and with a glob metacharacter in the path, and zero elements when the variable is empty. The claimed failure ("fresh-clone/worktree sweeps fail despite having a valid manifest") therefore has no path to occur. Additionally, `janitor-sweep` already ships the array-safe shape Copilot asks it to adopt: `LIST_ARGS` is an array and every call site expands it as `"${LIST_ARGS[@]}"`; only the assignment is written with the conditional expansion. Recorded as a false positive on the mechanism while the two-line style change is still recommended above — the two are not in tension.

## Implementation
**Status**: complete
**When**: 2026-09-14 10:47 -04:00
**By**: Claude Code Agent (Claude Opus)

**PR**: #625 at `d22f0d9`
**Addressed**: `## Integrated Review` (2026-09-14 10:34 -04:00, PR #625 @ `d7b8baf`) — both open actions
**Commits**: `bdb302b`, `d22f0d9`

### Actions
- [x] (must-fix) Retention block and report footer no longer claim the two clone caches are "safe to `rm -rf` at any time". Both now state the real condition — safe when no sweep or audit is running on this host — and the block carries `resolve_repo_checkout.sh:94-101`'s contract explicitly: the per-repo `flock` covers clone/refresh only and is released at script exit, so the caller reads its checkout unlocked and concurrent audits of the same repo on one host are not safe. The consequence of getting it wrong is named (a phantom `MISSING: no root AGENTS.md` indistinguishable from a real finding), as is why the fix is documentation: closing the window would need a lock held for the audit's duration, a change to the resolver's caller contract that is out of scope for a report-only skill. No shared lifecycle lock was added — `.claude/skills/janitor-sweep/SKILL.md` (Retention block; report-footer template) — `bdb302b`
- [x] (must-fix, other sites) Grepped the tree for the claim (`-iE "safe to (\`?rm -rf\`?|delete)|at any time"`). Two live sites, both in janitor-sweep's SKILL.md, both fixed above. `redact.sh`'s header does **not** carry the claim — its lines 25-27 are the "hygiene, not a security boundary" note, a different assertion; nothing to change there. The scratchpad README **did** repeat the whole-directory-delete claim without the condition ("the safe reclaim is deleting a **whole** directory"); corrected with the same caveat — `.agent/scratchpad/README.md:90-96` — `bdb302b`
- [x] (cosmetic) Both `${VAR:+--config-dir "$VAR"}` array-assignment sites converted to explicit array elements, in the shape janitor-sweep already uses for `LIST_ARGS` at its other call sites; `issue-triage` gains the `LIST_ARGS` array it lacked, so the two snippets now read identically — `.claude/skills/issue-triage/SKILL.md:101-107`, `.claude/skills/janitor-sweep/SKILL.md:148-155` — `d22f0d9`

  **The original was correct bash.** The inner quoted expansion inside `${VAR:+…}` is exempt from word splitting, so a `$ROOT` containing a space was never split. Nothing was broken and no behaviour changed; the change is for reviewer legibility, after three review rounds stopped on the construct. The word-splitting mechanism asserted by Copilot R3 and by this branch's round-5/6 local-review entries — and carried into issue #626 — is **false**, and is recorded here as false rather than as a fixed defect. Written as an `if` guard rather than a trailing `[ -n … ] && …` so the guard cannot become a status-1 final statement under `set -e`.

### Notes
- **Owed to the host (not doable from this worktree)**: the Integrated Review asked that the untrue word-splitting wording also be corrected where it is durable outside this branch — issue **#626**. That needs a GitHub comment/edit and is left to the host. The prior round-5/6 progress entries are historical appends and were not rewritten; this entry is the correction of record.
- `.agent/work-plans/issue-569/plan.md` describes neither the retention/footer text nor the array shape, so no plan sync was required.

### Verification
- `.agent/scripts/tests/run_script_tests.sh` — all shell suites ✅, 220 pytest passed
- `pre-commit run --all-files` — all hooks Passed (shellcheck, black, flake8, pylint, yamllint included)
- Both edited shell snippets extracted and parsed with `bash -n`
