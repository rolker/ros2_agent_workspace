...
| `AGENTS.md` | Script-reference rows for `resolve_repo_checkout.sh`, `workspace_root.sh`, `manifest_fallback.sh`, `redact.sh` and `field_mode.sh` (instruction file — **operator-approved**). The edits are **descriptive only**: five Script Reference rows, plus — under § Worktree Workflow → Field Mode (`AGENTS.md:193-208`) — an `is_field_url` usage block and a paragraph stating that an empty url classifies as dev mode. That section addition documents an existing helper's semantics for callers; it changes no rule, boundary, permission or procedure, and field mode itself is decided exactly as before. Round 5 fixes — the `manifest_fallback.sh` row states the url-identity comparison and the source-time exit 5, and both exit-5 enumerations name the causes the scripts' headers list |
