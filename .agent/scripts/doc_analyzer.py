...
REPORT_FILE = BASE_DIR.parent / '.agent' / 'scratchpad' / 'documentation_quality_report.md'
        ...
        try:
            ...
            for header_regex in KEY_HEADERS:
                if re.search(header_regex, content, re.IGNORECASE):
                    details['headers_found'].append(header_regex.replace(r'#\s+', '').replace(r'\\', ''))
            
            header_score = min(WEIGHT_HEADERS, len(details['headers_found']) * 5)
            score += header_score
            if header_score < WEIGHT_HEADERS:
                 details['issues'].append(f"Missing key sections (Found {len(details['headers_found'])}/4+ needed for max score)")
...
def generate_report(results):
    ...
    for res in sorted_results:
        ...
        lines.append(f"- **package.xml**: Description len: {res['pkg_desc_len']}, License: {'✅' if res['license_found'] else '❌'}")
        lines.append(f"- **README**: {'✅' if res['readme_exists'] else '❌'} ({res['readme_lines']} lines)")
