...
11. **Link GitHub references on first mention** — in summaries/reports, include a clickable URL for each issue, PR, commit, or repository when first mentioned. Use markdown links where supported; otherwise include the full URL inline or on the next line. This is a link rule only, not a required summary template.
