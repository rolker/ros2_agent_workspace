...
help:
	...
	@echo "  merge-pr (ISSUE=<n> [REPO=<slug>] | PR=<n> REPO=<slug>) [NO_WAIT=1] - Merge a PR, clean up worktree/branches, then sync (PR= needs REPO=, e.g. REPO=workspace)"
	@echo "                  NO_WAIT=1 skips ALL pre-merge verification: the CI wait, the ci-local attestation lookup, and its push"
	@echo "                  Error 42 = merged, but the repo had no CI at the head and no ci-local attestation (see AGENTS.md 'Merge verification')"
