...
Where `<repo-name>` is a directory under `layers/main/*/src/`.
...
Find the repo under `layers/main/*/src/<repo-name>`. Verify it exists and
has at least one `package.xml`.

```bash
REPO_DIR=$(find layers/main/*/src/<repo-name> -maxdepth 0 -type d 2>/dev/null | head -1)
```
...
```bash
cd "$REPO_DIR" && git symbolic-ref refs/remotes/origin/HEAD 2>/dev/null | sed 's|refs/remotes/origin/||'
```
...
**Model**: `$AGENT_MODEL`
EOF
gh issue create -R <owner>/<repo> --title "Onboard repo with workspace standards" \
  --label "enhancement" --body-file "$BODY_FILE"
rm "$BODY_FILE"
```

**Create a layer worktree** on the project repo using the new issue number:
```bash
.agent/scripts/worktree_create.sh --issue <N> --type layer --layer <layer> --packages <repo-name>
...
gh pr create --title "Onboard repo with workspace standards" \
  --body-file <body_file> -R <owner>/<repo>
```
