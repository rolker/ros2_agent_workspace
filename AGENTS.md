...
| `.agent/scripts/local_review.sh` | Cross-model review of a diff via a local Ollama model (default `qwen3.5:4b-q8_0`, sized to stay fully GPU-resident on an 8 GB card); used by review-code specialist 5f, standalone-capable offline/field (#570). Chunks oversized diffs per file/hunk/line-range and sizes each request's `num_ctx` per chunk; `--plan` shows the chunk plan without contacting the server. Sizing method and model-selection evidence in [`.agent/knowledge/local_model_sizing.md`](.agent/knowledge/local_model_sizing.md) (#605) |
| `.agent/scripts/setup_ollama_kv_cache.sh` | Writes a systemd drop-in setting `OLLAMA_KV_CACHE_TYPE` (halves KV-cache VRAM) for a VRAM-constrained Ollama host — Ollama 0.32.0 has no per-request override (#605). Idempotent; **dry-run by default and refuses to act without an explicit `--yes`**: it writes under `/etc` with sudo and restarts a service, which needs the operator's specific sign-off. Not applied on any host — see [`.agent/knowledge/local_model_sizing.md`](.agent/knowledge/local_model_sizing.md) |
