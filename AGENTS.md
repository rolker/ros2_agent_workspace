...
| `.agent/scripts/progress_append.sh` | Prompt-free progress.md entry appender + scoped committer (`progress_append.sh [-C <dir>] <N> [--title <t>] < entry.md`); validates the ADR-0013 `## <Entry Type>` heading, creates frontmatter, commits only that file under the agent identity (fails loud if unset); allowlisted in tracked `.claude/settings.json` (#594) |
