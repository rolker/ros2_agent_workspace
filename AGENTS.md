...
| `.agent/scripts/resolve_repo_checkout.sh` | Resolve a project repo to a usable checkout **without assuming `layers/` exists** (`resolve_repo_checkout.sh <repo-name>`): an existing `layers/main/*/src/<repo>` wins, otherwise a shallow clone of the manifest URL into `<main-workspace-root>/.agent/scratchpad/janitor-repos/` (the cache is anchored at the MAIN root, so worktrees on a host share it). Prints `path<TAB>layer|clone` on stdout — `clone` tells the caller that layer-dependent checks (a `colcon test` run, "is it in the expected layer?") must report SKIPPED, never OK. Used by `audit-project` and `janitor-sweep`. **Every failure is named and distinct**: 2 = usage; **3 = no repo manifest configured at all** (`configs/manifest` missing — the state of every worktree and fresh clone, where `list_overlay_repos.py` prints an empty list at exit 0 and would otherwise read as "repo not found" or "nothing to audit", #609); 4 = repo not listed in a manifest that *was* read; 5 = clone/refresh failed; 6 = manifest unreadable |
