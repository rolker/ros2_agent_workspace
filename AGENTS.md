...
| `.agent/scripts/_resolve_work_plans_dir.sh` | Shared helper **(source)**: `resolve_work_plans_dir <N>` prints the issue's work-plans directory — `$WORK_PLANS_DIR_OVERRIDE`, else the worktree matched by `$WORKTREE_ISSUE`, else a basename glob for workspace worktrees; otherwise fails with the `worktree_enter.sh` remediation (#660) |
| `.agent/scripts/_resolve_default_branch.sh` | Shared helper **(source)**: `resolve_default_branch [<repo-root>]` prints a ref usable for `git diff <ref>...HEAD` — the local default branch, else `origin/<branch>`, falling back to `main` (#660) |
