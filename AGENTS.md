...
| `.agent/scripts/worktree_remove.sh` | Remove worktree. **(#659 round-2)** For a LAYER worktree, checks each non-symlinked package directory for a root `rosdep.yaml` BEFORE removal (same filter `rosdep_local_sources.sh`'s discovery uses); if any is found, regenerates the host-shared workspace-local rosdep sources dir (`rosdep_local_sources.sh <MAIN root>`, the same main-root anchoring `setup.bash` uses) AFTER removal succeeds — otherwise the removed worktree's key keeps resolving via the one `ROSDEP_SOURCE_PATH` every shell on the host shares, until something else happens to regenerate it. Deliberately does NOT run `rosdep update` (network, slow) — the cache refreshes on its own schedule. A regeneration failure is reported loudly (with the exact re-run command) but never turns a successful removal into a script failure — the worktree is already gone by that point. `merge_pr.sh` removes worktrees by calling this script directly, so the merge path is covered too |
