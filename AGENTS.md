...
| `.agent/scripts/_agy_review.sh` | Helper for `cross_model_review.sh`: one headless `agy` (Gemini CLI successor) review turn, prompt over stdin, result event validated — an empty response or quota/auth error is a failed review, not review-looking text (#660, verbatim port) |
