...
| `.agent/scripts/sync_repos.py` | Sync all workspace repositories (includes git-bug). Classifies each repo `SYNCED`/`SKIPPED`/`FAILED` and names the cause per repo in the summary. `SKIPPED` (exit 0): dirty tree, detached HEAD, or an optional layer (`configs/manifest/optional_layers.txt`) that is not set up on this host. **`FAILED` (exits 1)**: failed pull/fetch, a working tree or git state that cannot be read, a required layer that is not set up, a repo missing from an imported layer, or no repos enumerable at all (missing `configs/manifest` — e.g. run from a worktree). git-bug bridge failures only warn (ADR-0010 graceful degradation). `merge_pr.sh` catches the non-zero exit and reports "merged, but sync failed" (its own exit 3) rather than aborting bannerless (#609) |
| `.agent/scripts/merge_pr.sh` | Merge a PR + remove worktree + delete branches + `make sync` (worktree/issue-keyed; also `make merge-pr`). **Exit codes**: 0 = merged, cleaned up and synced; 1 = failed before the merge (nothing irreversible happened); 2 = usage error; **3 = the merge and cleanup SUCCEEDED but the post-merge `make sync` failed** — do not retry the merge, re-run `make sync` alone (#609) |
